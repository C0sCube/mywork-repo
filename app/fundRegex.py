...
class FundRegex():
    ...
    HEADER_PATTERNS = {
            ...
            r"^(assets|fund_aum|aum|fund_size|aum_detail|portfolio_details).*": "aum",
            r"^fund_mana.*": "fund_manager",
            r"^(fund_infor|scheme|fund_details|fund_snapshot|fund_feat).*": "scheme_details",
            r'^portfolio$': "scheme_details",
            r"^(investment|about_the|data_as)": "investment_objective",
            r"^(scheme_statistics|portfolio_stats|risk|ratio|maturity|qualitative|quantitative|volatility|debt_quant|other_parameter|performance_attri|performance).*": "metrics",
