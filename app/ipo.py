...
class IpoTrading(Information):
    ...
    def get_json_response(self, url, headers): #generic function to get the data or no ??
        ...
        try:
            ...
            if e.response.status_code == 401:
                ...
                super().log("Unauthorized access. Retrying after 60 minutes...") # calling parent _init_ or some other function??
    ...
    def get_ipo_list(self):
        get_data = self.get_json_response(self.ipolistlink, self.headers) #func inside func call
        if not get_data:
            return []
 
        ipo_list = []
        for data in get_data: #must be according to structure of ipo api
            ipo_list.append({
    ...
    def get_ipo_details(self, ipo): #details of one under list of get_ipo_list
        symbol = ipo['symbol']
        ...
        try:
            while True:
                ...
                active_ipos = [
                    ...
                    if ipo['issue_start_date'] <= cur_date_str <= ipo['issue_end_date'] # what is this ?? 
                ]
 
                if not active_ipos: #this is i think bascially good night because no new ipo
                    print("No active IPOs at the moment.")
                    time_to_next_day = (datetime.datetime.combine(
                        datetime.date.today() + datetime.timedelta(days=1),
                        datetime.time.min
                    ) - cur_time).total_seconds()
                    time.sleep(time_to_next_day) #dynamic calculation!! gr8
                    continue
 
                for ipo in active_ipos: # basically each ipo under list of ipos have consolidated data which MIGHT get updated at a frequency on a day
                    ...
                    while cur_time < end_time: #for THIS ipo loop till time isnt finished
                        cur_time = datetime.datetime.now()
