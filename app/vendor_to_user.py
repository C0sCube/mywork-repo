...
class VendorMapper:
    ...
    def _generate_map_value(scheme_count:int,data:dict):
        ...
        record_value["load"] = [
            {
                "comment":data.get("load",{}).get("entry_load",""),
                "type":"entry"
            },
            {
                "comment":data.get("load",{}).get("exit_load",""),
                "type":"exit"
            }
        ]
        ...
        field_location = {
            ...
            "load_entry": page_number,
            "load_exit": page_number,
    ...
    def map_to_fink(self,data,filename):
        ...
        final_container = {
            "metadata":{
                ...
                "file_type":"",
                "process_date": f"{datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
            },
            "records":[]
        }
...
if __name__ == "__main__":
    ...
    input_folder = os.path.join(os.getcwd(), "sql_learn", "json", "MAR25DATA")
    output_folder = os.path.join(os.getcwd(),"data","mapped_output")
