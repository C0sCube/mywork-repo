...
tata_path = pdfReader.get_file_path(r'\files\TataFactSheet2024.pdf')
dry_path  = pdfReader.get_file_path(r'\output\DryRun.pdf')

#FUND DATASET
#flag, regex_fund_name, font_size, font_color
...
paths, fund_names, imp_pages = pdfReader.check_and_highlight(tata_path,indices, fund_data)
...
document_data = pdfReader.get_clipped_data(tata_path, pages, bbox, fund)
text_data = pdfReader.extract_span_data(document_data,[])
data_cond = [[5.0,6.0,8.0],-15570765,20.0] #sizes, color, set_size
